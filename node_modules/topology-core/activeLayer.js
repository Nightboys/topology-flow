  ...
  ActiveLayer.prototype.updateLines = function (pens) {
    ...
    for (var _i = 0, _a = this.data.pens; _i < _a.length; _i++) {
      ...
      for (var _b = 0, nodes_2 = nodes; _b < nodes_2.length; _b++) {
        ...
        var cnt = 0;
        if (line.from.id === item.id) {
          item.rotatedAnchors.map((v, j) => {
              if (line.from.anchorIndex == v.anchorIndex) {
                line.from.x = item.rotatedAnchors[j].x;
                line.from.y = item.rotatedAnchors[j].y;
              }
            })
        ...
        if (line.to.id === item.id) {
          item.rotatedAnchors.map((v, j) => {
              if (line.to.anchorIndex == v.anchorIndex) {
                line.to.x = item.rotatedAnchors[j].x;
                line.to.y = item.rotatedAnchors[j].y;
              }
            })
