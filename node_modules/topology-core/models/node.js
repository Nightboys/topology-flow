...
var Node = /** @class */ (function (_super) {
  ...
  Node.prototype.draw = function (ctx) {
    ...
    if (this.icon) {
      ...
      var r = 3,
        g = 5;
      ctx.beginPath();
      ctx.moveTo(rect.x + g, rect.y + g);
      ctx.arcTo(
        rect.x + rect.width - g,
        rect.y + g,
        rect.x + rect.width - g,
        rect.y + rect.height - g,
        r
      );
      ctx.arcTo(
        rect.x + rect.width - g,
      ...
      ctx.arcTo(rect.x + g, rect.y + rect.height - g, rect.x + g, rect.y + g, r);
      ctx.arcTo(rect.x + g, rect.y + g, rect.x + rect.width - g, rect.y + g, r);
      ...
      // var rect = this.getIconRect();
      // ctx.fillStyle = "#EAF1FC";
      // ctx.fillRect(rect.x + 5, rect.y + 5, rect.width - 10, rect.height * 2 / 3 + 10);
  ...
  Node.prototype.drawImg = function (ctx) {
    ...
    img.onload = function () {
      ...
      if (!_this.gif && gif) {
        _this.gif = true;
        le5le_store_1.Store.set('LT:addDiv', _this);
      }
    };
  };
  ...
  Node.prototype.calcRotateAnchors = function (angle) {
    ...
    for (var _i = 0, _a = this.anchors; _i < _a.length; _i++) {
      var item = _a[_i];
      this.rotatedAnchors.push(item.clone(item.props).rotate(angle, this.rect.center));
    }
  };
  ...
  Node.prototype.clacChildrenRect = function () {
    if (!this.children) {
      return;
    }
    for (var _i = 0, _a = this.children; _i < _a.length; _i++) {
  ...
  Node.prototype.animate = function (now) {
    ...
    if (rectChanged) {
      ...
      if (!this.animateAlone) {
        le5le_store_1.Store.set('LT:rectChanged', this);
      }
    }
    return '';
  };
